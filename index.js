...
const PLANE = process.argv[3]; // Now undefined if not provided
...
const planesToRun = PLANE ? [PLANE] : ['xy', 'xz', 'yz'];

function runProjection(plane) {
  const triangles = [];
  ...
  stream.on('data', (facet) => {
    if (
      facet.vertices &&
      facet.vertices.length === 3 &&
      facet.vertices.every(
        (v) =>
          v &&
          typeof v.x === 'number' &&
          typeof v.y === 'number' &&
  ...
  stream.on('end', () => {
    try {
      ...
      const outputFile = path.join(
        inputDir,
        path.basename(inputFile, path.extname(inputFile)) + `.${plane}.svg`
      );
      ...
      console.log(
        `SVG outline (with holes) created as ${outputFile} on the ${plane} plane.`
      );
    } catch (err) {
      console.error(
        `Failed to create SVG for ${plane} plane: ${err.message}`
      );
    }
  });

  stream.on('error', (err) => {
    console.error(`Error reading STL file for ${plane} plane: ${err.message}`);
  });
}

// Run for each requested plane
planesToRun.forEach(runProjection);